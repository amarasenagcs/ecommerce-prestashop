...
class CompareProduct extends ObjectModel
{
    ...
    public static function getCompareProducts($id_compare)
    {
        ...
        FROM `'._DB_PREFIX_.'wbfeature_compare` c
        LEFT JOIN `'._DB_PREFIX_.'wbfeature_compare_product` cp ON (cp.`id_compare` = c.`id_compare`)
    ...
    public static function addCompareProduct($id_compare, $id_product)
    {
        ...
        $id_compare = Db::getInstance()->getValue('
            SELECT `id_compare`
            FROM `'._DB_PREFIX_.'wbfeature_compare`
        ...
        if (!$id_compare) {
            ...
            $sql = Db::getInstance()->execute(
                'INSERT INTO `'._DB_PREFIX_.'wbfeature_compare` (`id_compare`, `id_customer`)
                VALUES (NULL, "'.($id_customer ? (int)$id_customer: '0').'")'
            );
            if ($sql) {
                $id_compare = Db::getInstance()->getValue(
                    'SELECT MAX(`id_compare`) FROM `'._DB_PREFIX_.'wbfeature_compare`'
                );
        ...
        return Db::getInstance()->execute(
            'INSERT IGNORE INTO `'._DB_PREFIX_.'wbfeature_compare_product` (`id_compare`, `id_product`, `date_add`, `date_upd`)
    ...
    {
        return Db::getInstance()->execute(
            'DELETE cp FROM `'._DB_PREFIX_.'wbfeature_compare_product` cp, `'._DB_PREFIX_.'wbfeature_compare` c
    ...
    public static function getNumberProducts($id_compare)
    {
        return (int)(Db::getInstance()->getValue('
            ...
            FROM `'._DB_PREFIX_.'wbfeature_compare_product`
            WHERE `id_compare` = '.(int)($id_compare)));
    }
    ...
    public static function cleanCompareProducts($period = null)
    {
        ...
        Db::getInstance()->execute(
            'DELETE cp, c FROM `'._DB_PREFIX_.'wbfeature_compare_product` cp, `'._DB_PREFIX_.'wbfeature_compare` c
            WHERE cp.date_upd < DATE_SUB(NOW(), INTERVAL 1 WEEK) AND
            c.`id_compare`=cp.`id_compare`'
        );
    }
