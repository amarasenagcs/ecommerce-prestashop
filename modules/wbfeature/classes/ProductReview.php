...
class ProductReview extends ObjectModel
{
    ...
    public static $definition = array(
        'table' => 'wbfeature_product_review',
    ...
    public static function getByProduct($id_product, $p = 1, $n = null, $id_customer = null)
    {
        ...
        $validate = Configuration::get('WBFEATURE_PRODUCT_REVIEWS_MODERATE');
        $p = (int)$p;
        $n = (int)$n;
        if ($p <= 1) {
            $p = 1;
        ...
        if (!Cache::isStored($cache_id)) {
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                ...
                (SELECT count(*) FROM `'._DB_PREFIX_.'wbfeature_product_review_usefulness` pcu WHERE
                pcu.`id_product_review` = pc.`id_product_review` AND pcu.`usefulness` = 1) as total_useful,
                (SELECT count(*) FROM `'._DB_PREFIX_.'wbfeature_product_review_usefulness` pcu WHERE
                pcu.`id_product_review` = pc.`id_product_review`) as total_advice, '.
                ((int)$id_customer ? '(SELECT count(*) FROM`'._DB_PREFIX_.'wbfeature_product_review_usefulness` pcuc WHERE pcuc.`id_product_review` = pc.`id_product_review` AND pcuc.id_customer = '.(int)$id_customer.') as customer_advice, ' : '').
                ((int)$id_customer ? '(SELECT count(*) FROM `'._DB_PREFIX_.'wbfeature_product_review_report` pcrc WHERE pcrc.`id_product_review` = pc.`id_product_review` AND pcrc.id_customer = '.(int)$id_customer.') as customer_report, ' : '').'
                IF(c.id_customer, CONCAT(c.`firstname`, \' \',  LEFT(c.`lastname`, 100)), pc.customer_name) customer_name, pc.`content`, pc.`grade`, pc.`date_add`, pc.title
                  FROM `'._DB_PREFIX_.'wbfeature_product_review` pc
    ...
    public static function getByCustomer($id_product, $id_customer, $get_last = false, $id_guest = false)
    {
        ...
        if (!Cache::isStored($cache_id)) {
            $results = Db::getInstance()->executeS(
                'SELECT * FROM `'._DB_PREFIX_.'wbfeature_product_review` pc
    ...
    public static function getGradeByProduct($id_product, $id_lang)
    {
        ...
        return (Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            FROM `'._DB_PREFIX_.'wbfeature_product_review` pc
            LEFT JOIN `'._DB_PREFIX_.'wbfeature_product_review_grade` pcg ON
            (pcg.`id_product_review` = pc.`id_product_review`)
            LEFT JOIN `'._DB_PREFIX_.'wbfeature_product_review_criterion` pcc ON
            (pcc.`id_product_review_criterion` = pcg.`id_product_review_criterion`)
            LEFT JOIN `'._DB_PREFIX_.'wbfeature_product_review_criterion_lang` pccl ON
            (pccl.`id_product_review_criterion` = pcg.`id_product_review_criterion`)
            WHERE pc.`id_product` = '.(int)$id_product.'
    ...
    public static function getReviewNumber($id_product)
    {
        ...
        $validate = (int)Configuration::get('WBFEATURE_PRODUCT_REVIEWS_MODERATE');
    ...
    public static function getByValidate($validate = '0', $deleted = false)
    {
        $sql  = '
            ...
            FROM `'._DB_PREFIX_.'wbfeature_product_review` pc
            LEFT JOIN `'._DB_PREFIX_.'customer` c ON (c.`id_customer` = pc.`id_customer`)
            LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (pl.`id_product` = pc.`id_product` AND
                pl.`id_lang` = '.(int)Context::getContext()->language->id.Shop::addSqlRestrictionOnLang('pl').')
    ...
    public function validate($validate = '1')
    {
        ...
        $success = (Db::getInstance()->execute(
            'UPDATE `'._DB_PREFIX_.'wbfeature_product_review` SET
    ...
    public static function deleteGrades($id_product_review)
    {
        ...
        return (Db::getInstance()->execute(
            'DELETE FROM `'._DB_PREFIX_.'wbfeature_product_review_grade` WHERE `id_product_review` = '.(int)$id_product_review
        ));
    }
    ...
    public static function deleteReports($id_product_review)
    {
        ...
        return (Db::getInstance()->execute(
            'DELETE FROM `'._DB_PREFIX_.'wbfeature_product_review_report` WHERE `id_product_review` = '.(int)$id_product_review
        ));
    }
    ...
    public static function deleteUsefulness($id_product_review)
    {
        ...
        return (Db::getInstance()->execute(
            'DELETE FROM `'._DB_PREFIX_.'wbfeature_product_review_usefulness` WHERE `id_product_review` = '.(int)$id_product_review
        ));
    }
    ...
    {
        return (Db::getInstance()->execute(
            'INSERT INTO `'._DB_PREFIX_.'wbfeature_product_review_report` (`id_product_review`, `id_customer`)
    ...
    public static function isAlreadyReport($id_product_review, $id_customer)
    {
        return (bool)Db::getInstance()->getValue(
            'SELECT COUNT(*)
            FROM `'._DB_PREFIX_.'wbfeature_product_review_report`
    ...
    {
        return (Db::getInstance()->execute(
            'INSERT INTO `'._DB_PREFIX_.'wbfeature_product_review_usefulness` (`id_product_review`, `usefulness`, `id_customer`)
    ...
    public static function isAlreadyUsefulness($id_product_review, $id_customer)
    {
        return (bool)Db::getInstance()->getValue(
            'SELECT COUNT(*)
            FROM `'._DB_PREFIX_.'wbfeature_product_review_usefulness`
            WHERE `id_customer` = '.(int)$id_customer.'
            AND `id_product_review` = '.(int)$id_product_review
        );
    }
    ...
    public static function getReportedReviews()
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            FROM `'._DB_PREFIX_.'wbfeature_product_review_report` pcr
            LEFT JOIN `'._DB_PREFIX_.'wbfeature_product_review` pc
                ON pcr.id_product_review = pc.id_product_review
            LEFT JOIN `'._DB_PREFIX_.'customer` c ON (c.`id_customer` = pc.`id_customer`)
            LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON
            (pl.`id_product` = pc.`id_product` AND
                pl.`id_lang` = '.(int)Context::getContext()->language->id.' AND
                pl.`id_lang` = '.(int)Context::getContext()->language->id.Shop::addSqlRestrictionOnLang('pl').')
            ORDER BY pc.`date_add` DESC'
        );
    }
