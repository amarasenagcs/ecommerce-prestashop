...
class wbBlog extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->version = '1.0.0';
        $this->author = 'Webiarch';
        $this->bootstrap = true;
        $this->need_upgrade = true;
        $this->controllers = array('archive','single');
        parent::__construct();
        $this->displayName = $this->l('Webiarch Blog Module');
        $this->description = $this->l('wbblog Powerfull Prestashop Blog Module by Webiarch');
    ...
    public function addQuickAccessLink()
    {
        ...
        if (isset($languages) && !empty($languages)) {
            foreach ($languages as $language) {
                $QuickAccess->name[$language['id_lang']] = $this->l("wbblog Settings");
